...
} from '../src/main/knowledge-processing/run-workspace'
...
function plan(contents: string[], limit: number, attachmentAt?: number) {
  return planKnowledgeRunWorkspace(
...
describe('Knowledge Run workspace plan', () => {
  ...
  it('creates a self-describing input guide inside this Run only', () => {
  ...
  it('rejects attachment bytes that do not match their metadata', () => {
    ...
    expect(() => planKnowledgeRunWorkspace(input, 'raw:test', 100))

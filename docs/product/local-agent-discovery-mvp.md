...
这种按需模型避免为大型外部历史维护一份容易过期的正文镜像，同时保留来源身份和重新核查的可能。Task 接受输入后可以在自身 Git revision 中物化加工所需的活动页、证据页和附件；这些文件记录当次工作，不改变外部来源的所有权。
...
当前 JSON catalog、同步扫描和按需读取是早期规模取舍。只有真实数据证明需要时，才增加分页、增量索引、Worker 或数据库。正式 Knowledge 的 provenance 是上层核心承诺，但其结构不由发现模块决定。

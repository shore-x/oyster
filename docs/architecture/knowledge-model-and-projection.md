...
3. **投影层（在当前意图下什么重要）**：在用户给定的 Attention 下，把知识编译为便于人和 Agent 使用的 Markdown 风格视图。

这三层的控制权形成一个梯度：越靠近底层，流程越固定、来源约束越强；越靠近上层，越受用户意图、关注范围、表达粒度和任务目标控制。

中间层不应在 MVP 阶段被实现为一个预先设计好的“万能知识图谱”。系统只需要理解少量结构性关系：知识依据什么、如何延续或修订、如何被投影。`冲突`、`因果`、`相似`、`支持`、`概括`等语义关系本身也可能是需要证据和修订的知识，不应默认成为不可质疑的系统边。
...
投影是知识在某个用户 Attention 下的可消费表达。Attention 不是一个主题标签，而是用户对选择范围、问题方向、抽象程度、更新策略和表达形式的组合意图。

投影层可以表现为 Markdown 文档、项目概览、决策脉络、失败经验、给第三方 Agent 的 Context Packet 等。它不是新的真相来源，也不是唯一的规范性 Wiki。同一知识集合可以同时生成多个有效投影：

- **Attention 不变、知识增加**：生成该投影的新修订；
- **知识不变、Attention 改变**：生成并列的另一个投影，而不是覆盖原投影。

用户对投影的修改或反馈若要进入知识库，应作为新的观察，再经知识加工形成新的理解。这样可避免 `Knowledge → Projection → Knowledge` 的无出处自我强化。
...
flowchart BT
  ...
  K2 --> P1["Projection under Attention A"]
  K3 --> P1
  K2 --> P2["Projection under Attention B"]
  P1 -->|"user feedback becomes new observation"| E2["New Evidence"]
```
...
- 一个知识依据哪些观察或已有知识；
- 一个新理解如何延续、限定或修订旧理解；
- 一个投影由哪些知识、在哪个 Attention 下生成。

这些关系支撑出处、重建、修订和删除，是比领域本体更稳定的结构性不变量。
...
设计原则：**中间关系网络的一个重要价值，是支撑更高层的全局综合，而不只是逐块召回。** 但社区报告是面向一类查询预先生成的有损摘要，且更适合相对静态语料。对持续到来的 Agent 活动，它更适合作为可重建投影或索引，而不是唯一知识真相。
...
[The Living Wiki](https://openreview.net/attachment?id=e64EcfHp8L&name=pdf)进一步把 immutable sources、agent-owned wiki 和 human-configured schema 分开。其实验同时给出一个重要反例：Wiki 能提高部分来源命中与实体聚合，但综合答案未必优于 RAG，持续重写还会产生断链。这说明**可读 Wiki 是有价值的编译结果，却不能替代证据和中间知识，也需要独立质量评测。**
...
5. **高层表达必然有损且与目标相关。** GraphRAG 社区报告、Hindsight mental model 和 Wiki 页面都在压缩底层信息；它们的质量只能相对于目标查询或用户 Attention 判断。
...
| Living/OpenWiki | 页面与链接 | 可读导航和综合 | Agent 持续 gardening | 用户 schema/attention + Agent | 使用直接，断链和无出处重写风险高 |
...
- 借鉴 GraphRAG：层级综合很有价值，但应作为可重建结果；
- 借鉴 Agent Wiki：用户 Attention 应成为生成可读视图的显式输入，Markdown 是合适的交换与阅读界面。
...
- 投影记录所用 Attention 与知识依赖，既可更新，也可并列生成其他关注方向的视图。
...
6. 投影不是证据，用户反馈通过新的观察重新进入加工链；
7. Attention 影响选择和表达，不反向改写底层知识；
8. 所有派生结果可在模型或管线变化后重建；
9. 正常加工遵循追加原则，但用户删除权始终优先。
...
- 同一 Attention 的增量更新能否保持投影稳定，是否会产生断链或含义漂移；
